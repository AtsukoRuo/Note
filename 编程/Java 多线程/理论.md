...
可见性描述这样一个问题：写操作何时被其他处理器观察到。如果程序存在可见性问题，那么就意味着某些线程读取到了旧数据。

## 重排序

重排序是对读写操作的一种优化，在不影响单线程程序正确性的情况下，提升程序的性能，它可以表现为以下几点：

1. 编译器对汇编代码的重排
2. 处理器的乱序执行、分支预测
3. 缓存系统对可见性的影响，即其他处理器对本处理器上写操作顺序的感知。
...
如果两个 Core 同时向总线进行广播来 Invalid 对方的 cache，总线会进行仲裁，最终结果会是只有一个 Core 广播成功，而另一个 Core 会失败。
...
- 在写之前我们先要得到缓存段的独占权，如果我们当前没有独占权，我们先要和其他处理器协商，这也需要一些时间。同理，在这种场景下让处理器闲着无所事事是一种资源浪费。
...
其中，X、Y 为共享变量，初值为 0；而 r1、r2 为局部变量。当 Processor0 执行 L2 时，Processor1 对于 Y 的写操作还停留在写缓冲器中，那么此时 Processor0 就读取到了 Y = 0 这个旧值。这对 L4 操作有着同样的问题。
...
Java内置锁的状态总共有4种，级别由低到高依次为：无锁、偏向锁、轻量级锁和重量级锁。状态会随着竞争的激烈而逐渐升级，而且是不可逆的过程。在 JDK 1.6 之前，Java 内置锁仅仅是一个重量级锁。在 JDK 1.6 之后，JVM 为了提高锁的获取与释放效率，引入了偏向锁和轻量级锁。
...
Java 虚拟机在实现 monitorenter 字节码（申请锁）和 monitorexit 字节码（释放锁）时需要借 一个原子操作（ CAS 操作 ），这个操作代价相对来说比较昂贵。在偏向锁中，Java 虚拟机会为每个对象维护一个偏好（ Bias ）。偏向锁在第一次被线程获得时，使用 CAS 操作将该线程的 ID 记录在 Mark Word 中，此后，这个线程后续无论是再次申请该锁还是释放该锁，都无须借助原先昂贵的原子操作。
...
当进入同步块时，o 对象从无锁状态转变为轻量锁。虽然默认开启了偏向锁，但是开启是**有延迟**的，大概 4s。原因是 JVM 内部的代码有很多地方用到了 synchronized，如果直接开启偏向，产生竞争就要有锁升级，会带来额外的性能损耗，所以就有了延迟策略。因此，我们来看下面这段代码：
...
此时，当进入同步块时，o 对象从无锁状态转变为偏向锁状态。可以通过参数 `-XX:BiasedLockingStartupDelay=0` 将延迟改为 0，但是**不建议**这么做。

![img](./assets/pianxiangsuo-20230830155452.png)
...
在线程 t2 中获取获取到了轻量级锁。由于锁升级不可撤销，这就导致 t1 再次获取偏向锁时，只能获取到轻量级锁。偏向锁像是“**一锤子买卖**”，只要偏向了某个线程，后续其他线程尝试获取锁，都会变为轻量级锁。但是还有例外情况，这就涉及到了 epoch 这个概念。epoch 是为以下场景优化的：

1. 一个线程创建了大量对象并执行了初始的同步操作，之后在另一个线程中将这些对象作为锁进行之后的操作。这种情况下，会导致大量的偏向锁撤销操作
...
以 class 为单位，为每个 class 维护一个偏向锁撤销计数器，只要 class 的对象发生偏向撤销，该计数器 `+1`，当这个值达到重偏向阈值（默认 20）时。JVM 就认为该 class 下的对象偏向锁有问题，因此会进行**批量重偏向（bulk rebias）**。每次发生批量重偏向时，就将该值加 1，同时遍历 JVM 中所有线程的栈：

- 找到该 class 所有**正处于加锁状态**的偏向锁对象，将其`epoch`字段改为新值
- class 中**不处于加锁状态**的偏向锁对象，保持 `epoch` 字段值不变
- 注意，那些为偏向阈值贡献的对象，已经升级为轻量级锁了，不再考虑。

这样下次获得锁时，对比当前对象的`epoch`值和 class 的`epoch`。此时如果 `markword` 有线程 ID，而还有其他锁来竞争：

- 如果 epoch 不一样，就算当前已经偏向了其他线程，也不会执行撤销操作，而是直接通过 CAS 操作将其`mark word`的线程 ID 改成当前线程 ID，这也算是一定程度的优化，毕竟没升级锁；
- 如果 epoch 一样，那锁自然是要升级的。

假设该 class 计数器继续增长，当其达到批量撤销的阈值后（默认 40）时，JVM 就认为该 class 的使用场景存在多线程竞争，会标记该 class 为不可偏向。之后对于该 class 的锁，直接走轻量级锁的逻辑。但是在第一阶梯到第二阶梯的过渡过程中，也就是在彻底禁用偏向锁之前，还会给一次改过自新的机会：

- 如果在距离上次批量重偏向发生的 25 秒之内，累计撤销计数达到 40，就会发生批量撤销（偏向锁彻底 game over）
...
1. 即便是可偏向状态的对象，一旦在同步块外调用 `Object::hashCode()` 或者`System::identityHashCode(Object)` ，进入同步块时就会直接使用轻量级锁。
...
引入轻量级锁的主要目的是在多线程竞争不激烈的情况下，通过 CAS 机制竞争锁减少重量级锁产生的性能损耗。重量级锁使用了操作系统底层的互斥锁（Mutex Lock），会导致线程在用户态和核心态之间频繁切换，从而带来较大的性能损耗。但是当竞争很激烈，CAS 尝试再多也是浪费 CPU，此时是不得不升级为重量级锁。

获取轻量级锁的过程：

1. JVM 首先将在抢锁线程的栈帧中建立一个锁记录（Lock Record）

2. 然后拷贝对象头中的 Mark Word 复制到锁记录中的 Displaced Mark Word。这是因为无锁状态下的 Mark Word 存储 Hashcode 等信息，而轻量级锁状态下的 MarkWord 只主要存储 ptr_to_lock_record，而没有保留 Hashcode 信息。所以在释放锁时，要还原无锁状态下的信息。因此，我们必须使用 Displaced Mark Word 来暂存无锁状态下的 markWord。

3. 拷贝成功后，抢锁线程将使用 CAS 自旋操作，尝试将对象头的 MarkWord 的 ptr_to_lock_record（锁记录指针）指向自己的锁记录。如果更新成功：再将锁记录的 owner 指针指向锁对象。

   ![这里写图片描述](./assets/51a1ee96c6064f1baa236b20dd4df897tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
在 JVM 中，每个对象都关联一个监视器，这里的对象包含 Object 实例和 Class 实例。监视器提供两种机制：
...
- Contention List（竞争队列）：它通过 CAS 操作（无锁结构）+ 自旋将请求失败的线程放在队首。并且只有 owner 线程才能从队尾将线程放入 EntryList 中
- Entry List（候选者队列 ）：保存有资格运行的线程
- Wait Set（阻塞队列）：保存因调用 wait 方法而被阻塞的线程。
- OnDeck：准备运行的 Thread，至多有一个。Owner 线程并不把锁直接传递给 OnDeck 线程，而是把锁竞争的权利交给OnDeck，OnDeck 需要重新竞争锁。下面会介绍这种锁被其他线程抢走的情况。
...
由于一个缓存行中可以存储多个变量的副本，因此，如果多个核的线程在操作同一个缓存行中的不同变量数据，那么就会出现频繁的缓存失效，即使在代码层面看这两个线程操作的数据之间完全没有关系。这种不合理的资源竞争情况就是伪共享（False Sharing）。
