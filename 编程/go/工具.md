...
每一种通信方式都使用 xxConn 结构体来表示，诸如IPConn、TCPConn等，这些结构体都实现了Conn接口，Conn接口实现了基本的读、写、关闭、获取远程和本地地址、设置 timeout 等功能。
...
- Scan 家族：从标准输入 os.Stdin 中读取数据，包括Scan()、Scanf()、Scanln()
- SScan 家族：从字符串中读取数据，包括Sscan()、Sscanf()、Sscanln()
...
type ConfigData struct {
	ProxyID  int32 `yaml:"ProxyID"`
	Period   int32 `yaml:"Period"`
	Hardware struct {		// 这个成员名要大写
		Memory []int32 `yaml:"Memory"`
		NIC    []int32 `yaml:"NIC"`
		CPU    int32   `yaml:"CPU"`
		GPU    []int32 `yaml:"GPU"`
		Disk   []int32 `yaml:"Disk"`
	}
}
...
func InitConfig() error {
	...
	if err != nil {
		fmt.Println("读取配置文件 config.yaml 失败")
		return err
	}
