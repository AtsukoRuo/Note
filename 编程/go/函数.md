...
xxxxxxxxxx10 1package main2​3import (4    "fmt"5    "moduledemo/mypackage"  // 导入同一项目下的mypackage包6)7func main() {8    mypackage.New()9    fmt.Println("main")10}go
...
我们直接说明过，在T类型的参数上调用一个`*T`的方法是合法的，编译器隐式的获取了它的地址，但这仅仅是一个语法糖，T 类型不拥有所有`*T`指针的方法。这对实现接口是有影响的，请见下面这个例子：
