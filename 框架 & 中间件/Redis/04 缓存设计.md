...
- Redis 内部维护一个定时任务，默认每秒运行10次（通过配置`hz`设置）来删除任务。
  1. 循环遍历全部 redisDb，从当前 redisDb 中随机抽取 20 个键值，如果发现过期就直接删除。
  2. 判断 20 个键值中的 25%（也就是5个）是否过期：
     1. 如果小于等于25%，则退出当前 redisDb 的循环，继续下一个redisDb。
     2. 如果大于25%，继续抽取 20 个键值进行循环。
        1. 若循环总时间超过25毫秒，先退出循环，然后进入快模式，即定时任务的执行周期下降到 1ms。
        2. 没超过 25 毫秒，则退出当前 redisDb 的循环，继续下一个redisDb
