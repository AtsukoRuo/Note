...
- `prefer-same-zone-eureka`
  - false：按照 service-url 下的 list 取第一个治理中心来注册，并和其维持心跳检测，不会再向 list 内其它的治理中心注册和维持心跳。只有在第一个注册失败的情况下，才会依次向其它的治理中心注册
  
  - true：先通过 region 取 availability-zones 内的第一个 zone，然后通过这个 zone 取 service-url 下的 list，并向 list 内的第一个治理中心进行注册和维持心跳，不会再向 list 内的其它的治理中心注册和维持心跳。
  
-  通过 `eureka.instance.metadata-map.zone` 来决定服务消费者和服务提供者分别属于哪个 `zone`。



Eurake 保证优先调用同一个 zone 内的服务，只有在同一个 zone 内的服务不可用时，才去调用其他 zone 下的服务。
...
- `ZoneAvoidanceRule`：根据服务所在区域（zone）的性能和服务的可用性（就是下面两个断言），来选择服务实例，在没有区域的环境下，该策略和轮询策略类似。
