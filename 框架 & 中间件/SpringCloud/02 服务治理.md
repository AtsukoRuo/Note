...
所谓微服务，是指完成某一业务功能的独立系统。一个微服务可以有多个实例，所谓实例，是指一个具体的服务节点。Eureka 通过实例的 `spring.appliacation.name` 配置项来确定该实例是属于哪一个微服务的。若配置了`eureka.instance.appname`，那么它的优先级比 `spring.appliacation.name` 的优先级更高。
...
- `instance-id`：标识该实例
  ...
  默认格式为 `${spring.cloud.hostname}:$(spring.application.name):$(server.port)`。

  直接使用默认配置即可，否则可能搜不到实例。
...
- 通过 `eureka.instance.metadata-map.zone` 来决定「服务消费者」和「服务提供者」分别属于哪个 `zone`。而「服务中心」通过自己暴露的地址 和 `service-url` 来间接确定自己的 Zone。
