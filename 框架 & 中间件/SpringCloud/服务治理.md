...
- xxxxxxxxxx20 1feign:2  httpclient:3    # 是否启用Apache HTTP Client作为底层连接（Ribbon的默认方法）4    enabled: true5    # 尝试连接超时时间6    connection-timeout: 20007    # 是否禁止SSL协议验证8    disable-ssl-validation: false9    # 连接重新尝试10    connection-timer-repeat: 200011    # 默认最大连接数12    max-connections: 10013    # 单个调用最大连接数14    max-connections-per-route: 3015    # HttpClient的存活时间，默认为900，单位通过time-to-live-unit配置16    time-to-live: 50017    # HttpClient的存活时间单位，默认为秒（second）18    time-to-live-unit: milliseconds19    # 当HTTP返回码为3xx（重定向）时，是否执行重定向操作，默认为true20    follow-redirects: falseyaml
- HTTP 域名要填写服务名
