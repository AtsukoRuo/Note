...
1. 熔断：当一个服务出现故障或超时，熔断器会打开并快速失败，拒绝后续的请求，避免请求堆积和资源耗尽
...
- 如果断路器状态为 OPEN，在等待一段时间（默认为60秒，可配置）后，断路器状态就会从 OPEN 转变为 HALF_OPEN。此时处于一个检测状态，当调用到了一定次数（默认为10 次，可配置）时，判断环形位缓冲区中的成功比例是否达到阈值（默认失败比例 50% 以上，可配置）。如果达到，断路器就会重新关闭（状态设置为 CLOSED ），放行请求；如果没有达到，就打开断路器（状态为 OPEN），阻隔调用。
...
从配置文件中，定义 Config 以及 CircuitBreaker：
