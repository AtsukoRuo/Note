...
- real_ip_header field：指定从哪个头部取真实的 IP 地址，如果如果设置从 `X-Forwarded-For` 中取，那么取最后一个 IP 
  ...
  - Context: http, server, location
- real_ip_recursive：是否过滤环回地址，即如果 `X-Forwarded-For` 最后一个地址与客户端地址相同，会过滤掉该地址。
...
优先级：proxy_cache_path > Expires > Cache-Control > proxy_cache_valid 

可以通过 proxy_ignore_headers 来忽略 Expires、Cache-Control 响应头的影响
