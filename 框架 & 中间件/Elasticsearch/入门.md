...
**分片（Shard）**是 ES 底层基本的读写单元。ES 先分割巨大的索引数据（也就是接下来介绍的文档），按照路由算法将它们分摊多台机器上，从而提高系统的吞吐量，这些分割后的数据就是一个个分片。分片在底层上就是多个在 Apache Lucene 中实现的段（索引）。行业最佳实践是将单个分片的大小控制在 50 GB 以内。
