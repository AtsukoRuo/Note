...
一般是将 MaterialApp 作为根组件，但 MaterialApp 会直接把屏幕大小的紧约束传递给home属性所引用的Widget。而`Scaffold`组件会以松约束的形式将该约束传递下去。
...
Column & Row 组件在主轴大小，取决于自身约束、children 的总尺寸，以及mainAxisSize属性。而它们的副轴大小，取决于他们所有子元素在副轴方向上的最大大小、CrossAxisAlignment、自身约束。
...
Stack 里的子组件在布局时会被分为「有位置」和「无位置」这2大类，而其中「有位置」是指被 Positioned 包裹的子组件。
...
- `fit`：控制 Stack 如何将自身约束传达给「无位置」的子组件。
  ...
  - `StackFit.passthrough`（穿透）：直接传递自身约束（父组件传递过来的约束）
  
- `alignment`：对于以下两类子Widget会应用该属性：
  
  - 「无位置」Widget
  - 「有位置」Widget，但并未设置水平或垂直方向的对齐方式，此时，未设置的方向会应用 alignment 属性
...
1. Stack首先会找到所有“无位置”的子组件，并向它们传入 fit 属性所设置的布局约束
2. 在得到全部「无位置」子组件所确定的最终尺寸后，Stack 会把自身尺寸匹配到其中最大的子组件的尺寸。如果Stack里不存在「无位置」的子组件，则Stack会尽量将自身尺寸设置为父级布局约束所允许的最大尺寸。
...
相比较`Offstage`组件，`Visbility`是从widget tree 中彻底将其子组件移除，来实现显示隐藏效果的。
...
- `maintainAnimation`：是否保持动画。如果设置为 false，Visibility 组件会调用TickerMode 组件将动画控制器所依赖的 ticker 关闭。如果要设置为 true，那么maintainState 必须设置为 true。
...
`IntrinsicWidth` 对应的渲染对象`RenderIntrinsicWidth`在`performLayout`方法中会调用各个子渲染对象的`computeMinIntrinsicWidth`等方法，并且这些方法只会被`RenderIntrinsicWidth`它自己调用。
